...
### Exemplo para os 2 primeiros (Act_AD2, Act_AD3) .. para fazer os outros, só copiar as próximas linhas e ir mudando o nome dos objetos
#Act_AD2
...
#Act_AD3
Act_AD3.gr <- GRanges(seqnames = as.character(Act_AD3$chr), ranges = IRanges(start = Act_AD3$start, end = Act_AD3$end), peak = as.character(Act_AD3$name)) 
vizinho.act_ad3 <- nearest(Act_AD3.gr, Mm.genes, ignore.strand=TRUE) #identifica o gene mais proximo de cada pico 
Act_AD3$gene_mais_prox <- as.character(gene[vizinho.act_ad3,"SYMBOL"]) #adiciona o nome do gene mais do proximo ao pico correspondente
head(Act_AD3) #Visualizar o resultado

#### Pra ver os genes em comum entre dois cenários diferentes, por exemplo entre Act_AD2 e Act_AD3
intersect(as.character(Act_AD3$gene_mais_prox), as.character(Act_AD2$gene_mais_prox))
