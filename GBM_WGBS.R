options(scipen=999)
col=colorRampPalette(c("yellow","black","blue"),space="Lab")(75) #Palette
...
manifest <- read.delim("/media/data1/thais.projects/GCIMP-low/GBM.WGBS/file_manifest.txt")
manifest <- manifest[manifest$File.Name %in% list.files("/media/data1/thais.projects/GCIMP-low/GBM.WGBS/Methylation_-_Bisulfite_Sequencing/JHU_USC__IlluminaHiSeq_WGBS/Level_3/"),]
...
save(TCGA.06.0128,TCGA.16.1460,TCGA.19.1788,file="/home/thais/Cloud_SanDiego/TCGA/GBM_Methyl_Seq /IDHmut_3samples.rda")
...
p.27k <- GRanges(seqnames = paste0("chr",as.character(idh.mut.27k$Chromosome)),ranges = IRanges(start = idh.mut.27k$Genomic_Coor, end=idh.mut.27k$Genomic_Coor),probeID=as.character(idh.mut.27k$poposite.El))
...
gr <- GRanges(seqnames = "chr2",ranges = IRanges(start = 85537579, end=85538201))
...
b <- ctcf.all #objeto que você tem as probes a serem classificadas
...
ggplot(c,aes(x=newStart,y=value,color=variable))  +
  ...
  scale_color_manual(values=c("darkgreen","firebrick","firebrick","gray","gray","#ffa500","#ffa500","cyan"),
                     labels=c("GCIMP-low","GCIMP-high","GCIMP-high","Non-tumor Brain","Non-tumor Brai","Mesenchymal-like","Mesenchymal-like","LGm6-GBM"),
                     name="Legend")
#+
 # scale_x_continuous(breaks=arithSeq(-5000,5000))
...
real.ctcf <- read.table("/media/data1/thais.projects/GCIMP-low/CTCF_Flavahan/CTCF_predicted_hg19.bed")
colnames(real.ctcf) <- c("chrom","start","end","id","score","strand")
...
aux.GR <- GRanges(seqnames = as.character(a$chrom),ranges = IRanges(start = a$start-500000, end=a$start))
aux2.GR <- GRanges(seqnames = as.character(a$chrom),ranges = IRanges(start = a$end, end=a$end+500000))
aux.GR <- c(aux.GR,aux2.GR)
...
b <- subset(volcano,threshold %in% c("5"))
symbol <- rownames(b)
normal.mrna <- rna.seq.LGG.GBM.normalBrain[rownames(rna.seq.LGG.GBM.normalBrain) %in% symbol,1:5]
normal.mrna <- normal.mrna+1
normal.mrna <- log2(normal.mrna)
...
Heatmap(as.matrix(all), name = "Oncogenes e genes supressores tumorais", col=greenred(75), cluster_columns = FALSE,cluster_rows = TRUE,top_annotation = ha1,row_names_gp = gpar(fontsize = 8),show_column_names = FALSE)
dev.off()
...
volcano.map <- rna.seq.LGG.GBM.normalBrain[rownames(rna.seq.LGG.GBM.normalBrain[rna.seq.LGG.GBM.normalBrain$entrezID %in% intersect(as.character(rna.seq.LGG.GBM.normalBrain$entrezID),as.character(genes$gene_id)),]),]
...
volcano.map$symbol <- rownames(volcano.map)
...
#Dp71 and Dp140 - DMD gene
all.450k$Chromosome <- paste0("chr",as.character(all.450k$Chromosome))

a <- AnnotationTrack(start=31000000, end=33000000,chromosome="chrX", genome="hg19", name="AnnotationTrack")
gtrack <- GenomeAxisTrack()
itrack <- IdeogramTrack(genome = "hg19", chromosome = "chrX")

knownGenes <- UcscTrack(genome = "hg19", chromosome = "chrX", track = "knownGene", from = 31000000, to = 33000000, trackType = "GeneRegionTrack",rstarts = "exonStarts", rends = "exonEnds", gene = "name",symbol = "name", transcript = "name", strand = "strand",fill = "#8282d2", name = "UCSC Genes")

#symbol(knownGenes) <- c("HOXD4","HOXD4","HOXD3","HOXD3","HOXD3","HOXD-AS1","HOXD-AS1","HOXD-AS1","HOXD-AS1","HOXD-AS1","HOXD-AS1","HOXD1","HOXD1","HOXD1","HOXD1","HOXD1")

cpgIslands <- UcscTrack(genome = "hg19", chromosome = "chrX",track = "cpgIslandExt", from = 31000000, to = 33000000,trackType = "AnnotationTrack", start = "chromStart",end = "chromEnd", id = "name", shape = "box",fill = "#006400", name = "CpG Islands")

gr <- GRanges(seqnames = "chrX",ranges = IRanges(start = 31000000, end=33000000))

p.450k <- makeGRangesFromDataFrame(all.450k,
...
aux <- findOverlaps(p.450k,gr)
colnames(all.450k)[5:649] <- substr(colnames(all.450k)[5:649],1,12)
b <- all.450k[queryHits(aux),]
p.450k <- makeGRangesFromDataFrame(b,
                                   keep.extra.columns=FALSE,
                                   ignore.strand=TRUE,
                                   seqinfo=NULL,
                                   seqnames.field="Chromosome",
                                   ...
                                   starts.in.df.are.0based=FALSE)
anno.450k <- AnnotationTrack(p.450k, name = "450k probes")



aux <- as.character(subset(pd, clustM.supervised2 %in% c("G-CIMP-low"))$case.id)
low.mrna <- b[,colnames(b) %in% aux] #12

aux <- as.character(subset(pd, clustM.supervised2 %in% c("G-CIMP-high"))$case.id)
high.mrna <- b[,colnames(b) %in% aux] #240

aux <- as.character(subset(pd, clustM.supervised2 %in% c("Classic-like"))$case.id)
classic.mrna <- b[,colnames(b) %in% aux] #73

aux <- as.character(subset(pd, clustM.supervised2 %in% c("Mesenchymal-like"))$case.id)
mes.mrna <- b[,colnames(b) %in% aux] #101

aux <- as.character(subset(pd, clustM.supervised2 %in% c("PA-like-LGG"))$case.id)
palgg.mrna <- b[,colnames(b) %in% aux] #26

aux <- as.character(subset(pd, clustM.supervised2 %in% c("PA-like-GBM"))$case.id)
pagbm.mrna <- b[,colnames(b) %in% aux] #12

aux <- as.character(subset(pd, clustM.supervised2 %in% c("IDHmut-codel"))$case.id)
codel.mrna <- b[,colnames(b) %in% aux] #173

tudao <- cbind(b[,3:4],low.mrna,high.mrna,codel.mrna,classic.mrna,mes.mrna,pagbm.mrna,palgg.mrna)

p.450k <- makeGRangesFromDataFrame(tudao,
                                   keep.extra.columns=TRUE,
                                   ignore.strand=TRUE,
                                   seqinfo=NULL,
                                   seqnames.field="Chromosome",
...
dat1 <- DataTrack(p.450k,name="DNA Methylation %",col=c("orange3","purple","firebrick","darkgreen","darkblue","yellow","cyan"),groups=c(rep("GCIMP-low",12),rep("GCIMP-high",240),rep("Codel",173),rep("Classic-like",73),rep("Mesenchymal-like",101),rep("LGm6-GBM",12),rep("PA-like",26)),type="smooth")

plotTracks(list(itrack,gtrack,cpgIslands,knownGenes,dat1,anno.450k),showId=TRUE,legend=TRUE,cex=1.5,stacking="dense")

## CTCF qPCR - PDGFRA (Normalization by Flavahan et al, 2016. Nature)
a <- read.table("CTCF_PDGFRA.txt",header=TRUE)
c <- read.table("ctcf_am.txt",header=T)
b <- merge(as.data.frame(t(a[,-1])),as.data.frame(t(c[,-1])),by=0)
colnames(b) <- c("Sample","Untr12.1","PDGFRA","PSMB1","SPG11","Untr12.2","SCN4A.21459","SCN4A.2115")
b$mean.1 <- apply(b[,c(4:5)],1,mean)
b$PDGFRA_norm <- b$PDGFRA/b$mean.1
b$group <- c("GCIMP-low","GCIMP-high","GCIMP-low","GCIMP-low","GCIMP-high","GCIMP-high","GCIMP-high","GCIMP-high","GCIMP-low","GCIMP-high")
b$tumor.type
b$SCN4A.21459.norm <- b$SCN4A.21459/b$Untr12.2
b$SCN4A.2115.norm <- b$SCN4A.2115/b$Untr12.2
mean.low <- mean(b[b$group %in% "GCIMP-low","PDGFRA_norm"])
b$PDGFRA_relative <- b$PDGFRA_norm/mean.low

b$id <- as.factor(b$Sample)
b$id <- factor(b$id, levels = c("X7010","X2570","X1805","X0129","X7007","X7304","X6542","X6408","X7008","X0221"))
ggplot(b, aes(x = id,y=PDGFRA_relative-1,fill=group)) +
  geom_bar(stat="identity") + scale_y_continuous(labels=percent) +
  scale_fill_manual(values=c("firebrick","darkgreen"))


d <- melt(b[,c(1,10:13)],id=c("Sample","group"))
ggplot(d,aes(group,value,fill=group)) + geom_boxplot(outlier.shape=NA) + geom_jitter() + scale_fill_manual(values=c("firebrick","darkgreen")) + facet_wrap( ~ variable) + labs(title="CTCF")


c <- read.table("h3k27ac_am.txt",header=T)
b <-  as.data.frame(t(c[,-1]))
colnames(b) <- c("Untr12","ACTB.m145","GAPDH.pro")
b$group <-  c(rep("GCIMP-high",6),rep("GCIMP-low",4))
b$Sample <- rownames(b)
b$ACTB.m145.norm <- b$ACTB.m145/b$Untr12
b$GAPDH.pro.norm <- b$GAPDH.pro/b$Untr12

d <- melt(b[,c(4:7)],id=c("Sample","group"))
ggplot(d,aes(group,value,fill=group)) + geom_boxplot(outlier.shape=NA) + geom_jitter() + scale_fill_manual(values=c("firebrick","darkgreen")) + facet_wrap( ~ variable) + labs(title="H3K27ac")



c <- read.table("h3k4me3_am.txt",header=T)
b <-  as.data.frame(t(c[,-1]))
colnames(b) <- c("Untr12","GAPDH")
b$group <-  c(rep("GCIMP-high",8),rep("GCIMP-low",4))
b$Sample <- rownames(b)
b$GAPDH.norm <- b$GAPDH/b$Untr12

ggplot(b,aes(group,GAPDH.norm,fill=group)) + geom_boxplot(outlier.shape=NA) + geom_jitter() + scale_fill_manual(values=c("firebrick","darkgreen"))  + labs(title="H3K27ac")


b <- as.data.frame(t(a[,-1]))
b$mean <- apply(b[,3:4],1,mean)
b$PDGFRA_norm <- b$PDGFRA/b$mean
b$group <- c(rep("GCIMP-low",4),rep("GCIMP-high",6))
ggplot(b,aes(group,PDGFRA_norm_unt,fill=group)) + geom_boxplot(outlier.shape=NA) + geom_jitter() + scale_fill_manual(values=c("firebrick","darkgreen"))


#Fix IDHmut samples (min 5 reads)
load("/home/thais/Cloud_SanDiego/TCGA/GBM_Methyl_Seq /IDHmut_3samples.rda")
TCGA.06.0128 <- TCGA.06.0128[-1,]
TCGA.16.1460 <- TCGA.16.1460[-1,]
TCGA.19.1788 <- TCGA.19.1788[-1,]
...
TCGA.16.1460 <- subset(TCGA.16.1460, numCTreads > 4)
TCGA.19.1788 <- subset(TCGA.19.1788, numCTreads > 4)
TCGA.06.0128$chrom <- as.character(TCGA.06.0128$chrom)
TCGA.16.1460$chrom <- as.character(TCGA.16.1460$chrom)
TCGA.19.1788$chrom <- as.character(TCGA.19.1788$chrom)

TCGA.06.0128$start <- as.numeric(as.character(TCGA.06.0128$start))
TCGA.16.1460$start <- as.numeric(as.character(TCGA.16.1460$start))
TCGA.19.1788$start <- as.numeric(as.character(TCGA.19.1788$start))

all <- merge(TCGA.06.0128[,c("chrom","start","end","percentMeth","numCTreads")],TCGA.16.1460[,c("chrom","start","percentMeth","numCTreads")],by=c("chrom","start"),all.x=TRUE)
all <- merge(all,TCGA.19.1788[,c("chrom","start","percentMeth","numCTreads")],by=c("chrom","start"),all.x=TRUE)
colnames(all) <- c("chrom","start","end","percentMeth.0128","numCTreads.0128","percentMeth.1460","numCTreads.1460","percentMeth.1788","numCTreads.1788")
save(all, file="/home/thais/GBM WGBS/IDHmut_min5.rda")

load("/home/thais/GBM WGBS/Roadmap_Brain/normalBrain_GSE46644.rda")
normalBrain$chrom <- as.character(normalBrain$chrom)
tudao <- merge(all,normalBrain[,c(1,2,6,8,9)],by=c("chrom","start"),all.x=TRUE)
tudao$percentMeth.1788 <- as.numeric(as.character(tudao$percentMeth.1788))
tudao$percentMeth.1460 <- as.numeric(as.character(tudao$percentMeth.1460))
tudao$percentMeth.0128 <- as.numeric(as.character(tudao$percentMeth.0128))
tudao$end <- as.numeric(as.character(tudao$end))
save(tudao, file="/home/thais/GBM WGBS/IDHmut_min5_normalBrain.rda")
...
TCGA.14.1454 <- subset(TCGA.14.1454, numCTreads > 4)
TCGA.14.3477 <- subset(TCGA.14.3477, numCTreads > 4)
TCGA.14.1401$chrom <- as.character(TCGA.14.1401$chrom)
TCGA.14.1454$chrom <- as.character(TCGA.14.1454$chrom)
TCGA.14.3477$chrom <- as.character(TCGA.14.3477$chrom)

TCGA.14.1401$start <- as.numeric(as.character(TCGA.14.1401$start))
TCGA.14.1454$start <- as.numeric(as.character(TCGA.14.1454$start))
TCGA.14.3477$start <- as.numeric(as.character(TCGA.14.3477$start))

tudao <- merge(tudao,TCGA.14.1401[,c("chrom","start","percentMeth","numCTreads")],by=c("chrom","start"),all.x=TRUE)
tudao <- merge(tudao,TCGA.14.1454[,c("chrom","start","percentMeth","numCTreads")],by=c("chrom","start"),all.x=TRUE)
tudao <- merge(tudao,TCGA.14.3477[,c("chrom","start","percentMeth","numCTreads")],by=c("chrom","start"),all.x=TRUE)

colnames(tudao)[13:18] <- c("percentMeth.1401","numCTreads.1401","percentMeth.1454","numCTreads.1454","percentMeth.3477","numCTreads.3477")
save(tudao, file="/home/thais/GBM WGBS/IDHmut_min5_normalBrain_IDHwt.rda")
...
a <- AnnotationTrack(start=53735000, end=55400000,chromosome="chr4", genome="hg19", name="AnnotationTrack")
gtrack <- GenomeAxisTrack()
itrack <- IdeogramTrack(genome = "hg19", chromosome = "chr4")
...
cpgIslands <- UcscTrack(genome = "hg19", chromosome = "chr4",track = "cpgIslandExt", from = 53735000, to = 55400000,trackType = "AnnotationTrack", start = "chromStart",end = "chromEnd", id = "name", shape = "box",fill = "#006400", name = "CpG Islands")

aux.GR <- GRanges(seqnames = "chr4",ranges = IRanges(start = 53735000, end=55400000))

a <- tudao[tudao$chrom %in% "chr4",]
a.GR <- makeGRangesFromDataFrame(a,TRUE)
overlaps <- findOverlaps(a.GR, aux.GR)
a <- a[queryHits(overlaps),]
meth <- makeGRangesFromDataFrame(a[,c(1:4,6,8,10,11,13,15,17)],TRUE)
genome(meth) <- "hg19"
meth <- DataTrack(meth,name="DNA methylation",type="smooth",groups=c("GCIMP.low",rep("GCIMP.high",2),rep("Non-tumor",2),rep("Mesenchymal-like",2),"LGm6-GBM"),col=c("firebrick","darkgreen","blue","yellow","darkgrey"))
...
overlaps <- findOverlaps(makeGRangesFromDataFrame(lost), aux.GR)
b <- lost[queryHits(overlaps),]
ctcf.paper <- makeGRangesFromDataFrame(b,TRUE)
genome(ctcf.paper) <- "hg19"
ctcf.paper <- AnnotationTrack(ctcf.paper,name="CTCF (Flavahan, 2016)",col="black",fill="black")

plotTracks(list(itrack,gtrack,txTranscripts_v1,ctcf,ctcf.paper,meth),span=0.01,legend=TRUE,n=500,from=53735000,to=55400000,collapseTranscripts="shortest")

#zoom 
#plotTracks(list(itrack,gtrack,txTranscripts_v1,cpgIslands,ctcf,meth),span=0.05,legend=TRUE,from=54700000,to=54900000,collapseTranscripts="shortest")

#check DNA methylation 450k or merged set for CpG probes at this CTCF binding site
load("~/Cloud_SanDiego/TCGA/LGG.GBM/NewSamples_16-09/tudao.27.450.rda")
aux.GR <- GRanges(seqnames = "chr4",ranges = IRanges(start = 54700000, end=54900000))
ids.mut <- c("TCGA-06-0128","TCGA-16-1460","TCGA-19-1788")
ids.wt <- c("TCGA-14-1454","TCGA-14-3477","TCGA-14-1401")
...
b <- subset(volcano, Brain_TSG == TRUE)
#symbol <- rownames(b)[65:106] #Oncogene -> 1:29, 30:64, 65:106
symbol <- "SIM2" #Oncogene -> 1:29, 30:64, 65:106
normal.mrna <- rna.seq.LGG.GBM.normalBrain[rownames(rna.seq.LGG.GBM.normalBrain) %in% symbol,1:5]
normal.mrna <- normal.mrna+1
normal.mrna <- log2(normal.mrna)
aux <- as.character(subset(pd, clustM.supervised2 %in% c("G-CIMP-low"))$case.id)
...
#check DNA methylation 450k or merged set for CpG probes at this CTCF binding site
load("~/Cloud_SanDiego/TCGA/LGG.GBM/NewSamples_16-09/tudao.27.450.rda") #nothing on the merged set
load("~/Cloud_SanDiego/TCGA/LGG.GBM/NewSamples_16-09/all_450.rda")
rownames(all.450k) <- as.character(all.450k$Composite.Element.REF)
aux.GR <- GRanges(seqnames = "chr4",ranges = IRanges(start = 54700000, end=54900000))
p450k.GR <- GRanges(seqnames = paste0("chr",as.character(all.450k$Chromosome)),ranges = IRanges(start = all.450k$Genomic_Coordinate, end=all.450k$Genomic_Coordinate),probeID=as.character(all.450k$Composite.Element.REF))
overlaps <- findOverlaps(p450k.GR, aux.GR)
b <- all.450k[queryHits(overlaps),]
b <- b[with(b,order(Genomic_Coordinate)),]
colnames(b)[-c(1:4)] <- substr(colnames(b)[-c(1:4)],1,12)

aux <- as.character(subset(pd, clustM.supervised2 %in% c("G-CIMP-low"))$case.id)
a <- b[,colnames(b) %in% aux]
aux <- hclust(dist(t(a)))
ordem <- a[,aux$order]
...
aux <- hclust(dist(t(a)))
ordem <- cbind(ordem,a[,aux$order])
clab <- c(clab,rep("purple",ncol(a)))
aux <- as.character(subset(pd, clustM.supervised2 %in% c("Classic-like"))$case.id)
a <- b[,colnames(b) %in% aux]
aux <- hclust(dist(t(a)))
ordem <- cbind(ordem,a[,aux$order])
clab <- c(clab,rep("orange3",ncol(a)))
aux <- as.character(subset(pd, clustM.supervised2 %in% c("Mesenchymal-like"))$case.id)
a <- b[,colnames(b) %in% aux]
aux <- hclust(dist(t(a)))
ordem <- cbind(ordem,a[,aux$order])
clab <- c(clab,rep("yellow",ncol(a)))
aux <- as.character(subset(pd, clustM.supervised2 %in% c("PA-like-GBM"))$case.id)
a <- b[,colnames(b) %in% aux]
aux <- hclust(dist(t(a)))
ordem <- cbind(ordem,a[,aux$order])
clab <- c(clab,rep("darkblue",ncol(a)))
aux <- as.character(subset(pd, clustM.supervised2 %in% c("PA-like-LGG"))$case.id)
a <- b[,colnames(b) %in% aux]
aux <- hclust(dist(t(a)))
ordem <- cbind(ordem,a[,aux$order])
clab <- c(clab,rep("cyan",ncol(a)))



heatmap.plus(as.matrix(ordem),
             Colv=NA,
             Rowv=NA,
             col=jet.colors(75),
...
aux.GR <- GRanges(seqnames = "chr4",ranges = IRanges(start = 54828000, end=54830000)) #potential enhancer
a <- tudao[tudao$chrom %in% "chr4",]
a.GR <- makeGRangesFromDataFrame(a,TRUE)
overlaps <- findOverlaps(a.GR, aux.GR)
a <- a[queryHits(overlaps),]

clab <- cbind(c("gray","gray","firebrick","firebrick","darkgreen","yellow","yellow","darkblue"),c("gray","gray","firebrick","firebrick","darkgreen","yellow","yellow","darkblue"))

heatmap.plus(as.matrix(a[,c(10,11,6,8,4,13,15,17)]),
             Colv=NA,
             Rowv=NA,
             col=jet.colors(75),
             ColSideColors = clab,
...
a <- AnnotationTrack(start=42283356, end=42357476,chromosome="chr20", genome="hg19", name="AnnotationTrack")
gtrack <- GenomeAxisTrack()
...
library(org.Hs.eg.db)
txdb_hg19 <- TxDb.Hsapiens.UCSC.hg19.knownGene
txTranscripts_v1 <- GeneRegionTrack(txdb_hg19, genome="hg19", chromosome="chr20", showId=TRUE, geneSymbol=TRUE, name="UCSC")
symbols <- unlist(mapIds(org.Hs.eg.db, gene(txTranscripts_v1), "SYMBOL", "ENTREZID", multiVals = "first"))
symbol(txTranscripts_v1) <- symbols[gene(txTranscripts_v1)]

aux.GR <- GRanges(seqnames = "chr20",ranges = IRanges(start = 42283356, end=42357476))
overlaps <- findOverlaps(makeGRangesFromDataFrame(volcano.map), aux.GR)
b <- volcano.map[queryHits(overlaps),]
b <- b[with(b,order(seqnames,start)),]
txTranscripts_v1 <- GeneRegionTrack(makeGRangesFromDataFrame(b[,c("seqnames","start","end")]),symbol=as.character(b$symbol), genome="hg19", chromosome="chr20", showId=TRUE, geneSymbol=TRUE, name="Genes",fill=as.character(b$sig))

ctcf <- read.table("/media/data1/Random/DiffCTCF_hg19.bed")
colnames(ctcf) <- c("chrom","start","end")
overlaps <- findOverlaps(makeGRangesFromDataFrame(ctcf), aux.GR)
b <- ctcf[queryHits(overlaps),]
ctcf <- makeGRangesFromDataFrame(b,TRUE)
genome(ctcf) <- "hg19"
ctcf <- AnnotationTrack(ctcf,name="CTCF",col="black",fill="black")

h3k27ac <- read.table("/media/data1/Random/DiffH3K27ac_hg19.bed")
colnames(h3k27ac) <- c("chrom","start","end")
overlaps <- findOverlaps(makeGRangesFromDataFrame(h3k27ac), aux.GR)
b <- h3k27ac[queryHits(overlaps),]
h3k27ac <- makeGRangesFromDataFrame(b,TRUE)
genome(h3k27ac) <- "hg19"
h3k27ac <- AnnotationTrack(h3k27ac,name="H3K27ac",col="black",fill="black")


h3k4me3 <- read.table("/media/data1/Random/DiffH3K4me3_hg19.bed")
colnames(h3k4me3) <- c("chrom","start","end")
overlaps <- findOverlaps(makeGRangesFromDataFrame(h3k4me3), aux.GR)
b <- h3k4me3[queryHits(overlaps),]
h3k4me3 <- makeGRangesFromDataFrame(b,TRUE)
genome(h3k4me3) <- "hg19"
h3k4me3 <- AnnotationTrack(h3k4me3,name="H3K4me3",col="black",fill="black")

a <- LGG.GBM.WGBS[LGG.GBM.WGBS$chrom %in% "chr20",]
a.GR <- makeGRangesFromDataFrame(a,TRUE)
overlaps <- findOverlaps(a.GR, aux.GR)
a <- a[queryHits(overlaps),]
meth <- makeGRangesFromDataFrame(a[,c(1:4,6,8,10,11)],TRUE)
genome(meth) <- "hg19"
#meth <- DataTrack(meth,name="DNA methylation",type="smooth",groups=c("GCIMP.low",rep("GCIMP.high",2),rep("Non-tumor",2)),col=c("firebrick","darkgreen","grey"))
meth <- DataTrack(meth,name="DNA methylation",type="smooth",groups=c("GCIMP.low","GCIMP.high1","GCIMP.high2","Non-tumor1","Non-tumor2"),col=c("firebrick","firebrick","darkgreen","grey","grey"))
...
plotTracks(list(itrack,gtrack,txTranscripts_v1,ctcf,h3k27ac,h3k4me3,meth),span=0.02,legend=TRUE,n=500,from=42283356,to=42357476)

#RNA-seq
load("/media/data1/thais.projects/GCIMP-low/GBM.WGBS/LGG_GBM_nonTumorBrain_RNAseqNorm.rda")
load("/media/data1/thais.projects/GCIMP-low/GBM.WGBS/teste_lowXhigh_rnaseq_Newnorm.rda")
rna.seq.LGG.GBM.normalBrain[,-642] <- log2(rna.seq.LGG.GBM.normalBrain[,-642]+1)
identical(rownames(lowXhigh_RNAseq),rownames(rna.seq.LGG.GBM.normalBrain))
rna.seq.LGG.GBM.normalBrain$symbol <- rownames(rna.seq.LGG.GBM.normalBrain)
rna.seq.LGG.GBM.normalBrain$meanNormal <- apply(rna.seq.LGG.GBM.normalBrain[,1:5],1,mean,na.rm=TRUE)
rna.seq.LGG.GBM.normalBrain <- cbind(rna.seq.LGG.GBM.normalBrain,lowXhigh_RNAseq[,c(252:256)])
colnames(rna.seq.LGG.GBM.normalBrain)[645] <- "meanLow"
colnames(rna.seq.LGG.GBM.normalBrain)[646] <- "meanHigh"
save(rna.seq.LGG.GBM.normalBrain,file="/media/data1/thais.projects/GCIMP-low/GBM.WGBS/LGG_GBM_nonTumorBrain_RNAseqNorm.rda")

##Upregulated genes - n = 124 
a <- (rna.seq.LGG.GBM.normalBrain[rna.seq.LGG.GBM.normalBrain$DiffMean > 3 & !is.na(rna.seq.LGG.GBM.normalBrain$p.value.adj) & rna.seq.LGG.GBM.normalBrain$p.value.adj < 0.05 & !is.na(rna.seq.LGG.GBM.normalBrain$DiffMean),642:649])
write.table(as.character(a$symbol),quote=F,row.names=F,col.names = F,sep="\t",file="/media/data1/biosoftwares/MARGE/gene_list/Upreg_genes_124_FC_3.txt")


### GWAS
gwas <- read.table(text=c("rs12752552 chr1 64763616 64763616	GBM
rs11233250 chr11 82685972 82685972	GBM
rs2562152 chr16 73898 73898	GBM
rs10852606 chr16 50094961 50094961	GBM
rs2235573 chr22 38081923 38081923	GBM
rs4252707 chr1 204539019 204539019	non.GBM
rs12076373 chr1 243688645 243688645	non.GBM
rs7572263 chr2 208186862 208186862	non.GBM
rs11706832 chr3 66452557 66452557	non.GBM
rs11598018 chr10 103901557 103901557	non.GBM
rs7107785 chr11 96014174 96014174	non.GBM
rs10131032 chr14 32780875 32780875	non.GBM
rs3751667 chr16 954554 954554	non.GBM"))
colnames(gwas) <- c("ID","chrom","start","end","type")
all.os <- makeGRangesFromDataFrame(LGG.GBM.WGBS,TRUE)
distanceToNearest(makeGRangesFromDataFrame(gwas),all.os)

p450k <- read.table("/media/data1/hg38_450k.bed")
colnames(p450k) <- c("chrom","start","end","probeID")
p450k$end <- p450k$start
a <- distanceToNearest(makeGRangesFromDataFrame(gwas),makeGRangesFromDataFrame(p450k))
a <- as.data.frame(a)


require(XLConnect)
wb <- loadWorkbook(system.file("/home/thais/Downloads/PanGlioma_MethylationSignatures.xlsx", package = "XLConnect"))
...
mysheets <- read_excel_allsheets("/home/thais/Downloads/PanGlioma_MethylationSignatures.xlsx")

for(i in 1:nrow(gwas)) {
  res <- lapply(mysheets, function(ch) grep(as.character(gwas$ID)[i], ch))
 print(res) 
}



#### Genes with bHLH motif
bHLH.genes <- read.table("/media/data1/thais.projects/GCIMP-low/ActiveMotif_Data/homer.motifs/HOX/genes_with_bHLH_motif.txt")
sum(as.character(bHLH.genes$V1) %in% as.character(genes$gene_name)) #95 out of 108

heatmap.plus(as.matrix(na.omit(genes[genes$gene_name %in% as.character(bHLH.genes$V1),c("RNAseq8.meanNormal","RNAseq8.meanLow","RNAseq8.meanHigh")])),
             col = greenred(75),
             scale = "none",
             # labRow = NA,
             #labCol = NA,
             Colv = NA,
             #Rowv = NA,
             ColSideColors = cbind(c("grey","darkgreen","firebrick"),c("grey","darkgreen","firebrick"))
             #RowSideColors = rlab
...
######## CpG type annotation
CpGislands <- read.table("/media/data1/thais.projects/GCIMP-low/GBM.WGBS/CpGislands_hg38_april17.txt")
CpGislands <- CpGislands[,c(2,3,4)]
colnames(CpGislands) <- c("chrom","start","end")
LGG.GBM.WGBS$CpG_type <- "Open-sea"
aux <- findOverlaps(makeGRangesFromDataFrame(LGG.GBM.WGBS),makeGRangesFromDataFrame(CpGislands),type="within")
LGG.GBM.WGBS[queryHits(aux),"CpG_type"] <- "CpG island"
shore <- CpGislands[,c(1,2)]
shore$startS <- shore$start - 2000
shore <- shore[,c(1,3,2)]
aux <- CpGislands[,c(1,3)]
aux$end2 <- aux$end + 2000
colnames(shore) <- c("chrom","start","end")
colnames(aux) <- c("chrom","start","end")
shore <- rbind(shore,aux)
aux <- findOverlaps(makeGRangesFromDataFrame(LGG.GBM.WGBS),makeGRangesFromDataFrame(shore),type="within")
LGG.GBM.WGBS[queryHits(aux),"CpG_type"] <- ifelse(LGG.GBM.WGBS[queryHits(aux),"CpG_type"] %in% "CpG island","CpG island","CpG shore")
save(LGG.GBM.WGBS,file="/media/data1/thais.projects/GCIMP-low/GBM.WGBS/hg38_IDHmut_min5_normalBrain_IDHwt_CpGtype_mean.rda")


######## 2017
### 450k hg38 - versao GDC
hg38.450k <- read.delim("/home/thais/Downloads/jhu-usc.edu_ACC.HumanMethylation450.1.lvl-3.TCGA-OR-A5J1-01A-11D-A29J-05.gdc_hg38.txt")
hg38.450k <- hg38.450k[,c("Composite.Element.REF","Chromosome","Start","End")]
save(hg38.450k,file="/media/data1/450k_hg38_GDC.rda")

### 450k probes differentially methylated
load("/media/data1/thais.projects/GCIMP-low/GBM.WGBS/low_high_450.rda") #low: 12 e high: 240
low.high.450k <- merge(low[,-c(1:4)],high[,-c(1:4)],by=0)
rownames(low.high.450k) <- as.character(low.high.450k$Row.names)
low.high.450k <- low.high.450k[,-1] #12 lows and 240

WGBS.GR <- makeGRangesFromDataFrame(LGG.GBM.WGBS,TRUE)
DM.633.Fig3E <- read.table("/media/data1/thais.projects/GCIMP-low/GBM.WGBS/Homer_633DMP_G-CIMPlow_oct2015/homer.450m2.All.txt")
colnames(DM.633.Fig3E) <- c("chrom","start","end","id","unk")
aux <- strsplit(as.character(DM.633.Fig3E$id),"[.]")
DM.633.Fig3E$id <- sapply(aux, "[[", 2)
DM.633.Fig3E$CpG.type <- sapply(aux, "[[", 1)
DM.633.Fig3E.hg38 <- hg38.450k[as.character(hg38.450k$Composite.Element.REF) %in% as.character(DM.633.Fig3E$id),]
DM.633.Fig3E.hg38 <- merge(DM.633.Fig3E.hg38,DM.633.Fig3E[,c("id","CpG.type")],by.x="Composite.Element.REF",by.y="id")

aux <- findOverlaps(WGBS.GR,makeGRangesFromDataFrame(DM.633.Fig3E.hg38))
probes <- DM.633.Fig3E.hg38[subjectHits(aux),]
probes.599.450k <- low.high.450k[as.character(probes$Composite.Element.REF),]
probes.599.WGBS <- LGG.GBM.WGBS[queryHits(aux),]


h3k27ac.r <- dba.report(h3k27ac.a,th=1)
aux <- findOverlaps(makeGRangesFromDataFrame(probes),h3k27ac.r)
probes$H3K27ac <- 0
probes[queryHits(aux),"H3K27ac"] <- 1

h3k4me3.r <- dba.report(h3k4me3.a,th=1)
aux <- findOverlaps(makeGRangesFromDataFrame(probes),h3k4me3.r)
probes$H3K4me3 <- 0
probes[queryHits(aux),"H3K4me3"] <- 1

ctcf.r <- dba.report(ctcf.a,th=1)
aux <- findOverlaps(makeGRangesFromDataFrame(probes),ctcf.r)
probes$CTCF <- 0
probes[queryHits(aux),"CTCF"] <- 1


hmec.r <- dba.report(hmec.a,th=1)
seqlevels(hmec.r) <- paste0("chr",seqlevels(hmec.r))
aux <- findOverlaps(makeGRangesFromDataFrame(probes),hmec.r)
probes$hmec <- 0
probes[queryHits(aux),"hmec"] <- 1
probes$CpG.type <- factor(probes$CpG.type)
levels(probes$CpG.type) <- c("CpG island", "Open-sea", "CpG shore")


library(ComplexHeatmap)
ha1 = HeatmapAnnotation(df = data.frame(CpG.type=probes$CpG.type),col = list(CpG.type = c("CpG island" =  "deeppink1", "CpG shore" = "darkolivegreen2", "Open-sea" = "deepskyblue")),which = "row", width = unit(1, "cm"))
ha2 = HeatmapAnnotation(probes[,c(6:9)],which = "row",col = list(H3K27ac = c("0" =  "grey", "1" = "black"),H3K4me3 = c("0" =  "grey", "1" = "black"),CTCF = c("0" =  "grey", "1" = "black"),hmec = c("0" =  "grey", "1" = "black"))  ,width = unit(1, "cm"))
top_anno_450k <- HeatmapAnnotation(df = data.frame(Subtype=c(rep("G-CIMP low",12),rep("G-CIMP high",38))),
                                         col = list(Subtype = c("G-CIMP low" = "darkgreen", "G-CIMP high" = "firebrick3")))
top_anno_WGBS <- HeatmapAnnotation(df = data.frame(Subtype=c("G-CIMP low","G-CIMP high","G-CIMP high","Mesenchymal-like","Mesenchymal-like","LGm6-GBM","Non-tumor brain","Non-tumor brain")),
                                         col = list(Subtype = c("G-CIMP low" = "darkgreen", "G-CIMP high" = "firebrick3", "Mesenchymal-like" = "yellow","LGm6-GBM" = "cyan", "Non-tumor brain" = "gray")))


ha1 + 
  ha2 +
  Heatmap(probes.599.450k[,c(1:50)], name = "Beta-values", col=jet.colors(75), cluster_columns = FALSE,cluster_rows = TRUE,show_row_names=FALSE,show_column_names = FALSE,top_annotation = top_anno_450k) +
  Heatmap(probes.599.WGBS[,c(4,6,8,13,15,17,10,11)], name = "WGBS", col=jet.colors(75), cluster_columns = FALSE,show_row_names=FALSE,show_column_names = FALSE,top_annotation = top_anno_WGBS) 



## CpGs differentially methylated according to WGBS
LGG.GBM.WGBS$meanHigh <- apply(LGG.GBM.WGBS[,c("percentMeth.1460","percentMeth.1788")],1,mean,na.rm=T)
LGG.GBM.WGBS$Diff.low.high <- LGG.GBM.WGBS$percentMeth.0128 - LGG.GBM.WGBS$meanHigh

WGBS.FC <- LGG.GBM.WGBS[LGG.GBM.WGBS$Diff.low.high < -90 & !is.na(LGG.GBM.WGBS$Diff.low.high),]

h3k27ac.r <- dba.report(h3k27ac.a,th=1)
aux <- findOverlaps(makeGRangesFromDataFrame(WGBS.FC),h3k27ac.r)
WGBS.FC$H3K27ac <- 0
WGBS.FC[queryHits(aux),"H3K27ac"] <- 1

h3k4me3.r <- dba.report(h3k4me3.a,th=1)
aux <- findOverlaps(makeGRangesFromDataFrame(WGBS.FC),h3k4me3.r)
WGBS.FC$H3K4me3 <- 0
WGBS.FC[queryHits(aux),"H3K4me3"] <- 1

ctcf.r <- dba.report(ctcf.a,th=1)
aux <- findOverlaps(makeGRangesFromDataFrame(WGBS.FC),ctcf.r)
WGBS.FC$CTCF <- 0
WGBS.FC[queryHits(aux),"CTCF"] <- 1

hmec.r <- dba.report(hmec.a,th=1)
seqlevels(hmec.r) <- paste0("chr",seqlevels(hmec.r))
aux <- findOverlaps(makeGRangesFromDataFrame(WGBS.FC),hmec.r)
WGBS.FC$hmec <- 0
WGBS.FC[queryHits(aux),"hmec"] <- 1

#Homer
write.table(WGBS.FC[WGBS.FC$CpG_type %in% "CpG island",c("chrom","start","end","id")],row.names = F,col.names = F,quote=F,sep="\t",file = "/media/data1/thais.projects/GCIMP-low/ActiveMotif_Data/homer.motifs/WGBS_hypo_90FC/island.txt")
aux <- LGG.GBM.WGBS[abs(LGG.GBM.WGBS$Diff.low.high) < 10 & !is.na(LGG.GBM.WGBS$Diff.low.high) & LGG.GBM.WGBS$CpG_type %in% "CpG island",]
aux <- aux[sample(nrow(aux), 30000),]
write.table(aux[,c("chrom","start","end","id")],row.names = F,col.names = F,quote=F,sep="\t",file = "/media/data1/thais.projects/GCIMP-low/ActiveMotif_Data/homer.motifs/WGBS_hypo_90FC/bg_island.txt")

write.table(WGBS.FC[WGBS.FC$CpG_type %in% "CpG shore",c("chrom","start","end","id")],row.names = F,col.names = F,quote=F,sep="\t",file = "/media/data1/thais.projects/GCIMP-low/ActiveMotif_Data/homer.motifs/WGBS_hypo_90FC/shore.txt")
aux <- LGG.GBM.WGBS[abs(LGG.GBM.WGBS$Diff.low.high) < 10 & !is.na(LGG.GBM.WGBS$Diff.low.high) & LGG.GBM.WGBS$CpG_type %in% "CpG shore",]
aux <- aux[sample(nrow(aux), 30000),]
write.table(aux[,c("chrom","start","end","id")],row.names = F,col.names = F,quote=F,sep="\t",file = "/media/data1/thais.projects/GCIMP-low/ActiveMotif_Data/homer.motifs/WGBS_hypo_90FC/bg_shore.txt")

write.table(WGBS.FC[WGBS.FC$CpG_type %in% "Open-sea",c("chrom","start","end","id")],row.names = F,col.names = F,quote=F,sep="\t",file = "/media/data1/thais.projects/GCIMP-low/ActiveMotif_Data/homer.motifs/WGBS_hypo_90FC/opensea.txt")
aux <- LGG.GBM.WGBS[abs(LGG.GBM.WGBS$Diff.low.high) < 10 & !is.na(LGG.GBM.WGBS$Diff.low.high) & LGG.GBM.WGBS$CpG_type %in% "Open-sea",]
aux <- aux[sample(nrow(aux), 30000),]
write.table(aux[,c("chrom","start","end","id")],row.names = F,col.names = F,quote=F,sep="\t",file = "/media/data1/thais.projects/GCIMP-low/ActiveMotif_Data/homer.motifs/WGBS_hypo_90FC/bg_opensea.txt")

#Distance between WGBS FC and 633
aux <- distanceToNearest(makeGRangesFromDataFrame(DM.633.Fig3E.hg38),makeGRangesFromDataFrame(WGBS.FC))


library(ComplexHeatmap)
ha1 = HeatmapAnnotation(df = data.frame(CpG.type=WGBS.FC$CpG_type),col = list(CpG.type = c("CpG island" =  "deeppink1", "CpG shore" = "darkolivegreen2", "Open-sea" = "deepskyblue")),which = "row", width = unit(1, "cm"))
ha2 = HeatmapAnnotation(WGBS.FC[,c(23:26)],which = "row",col = list(H3K27ac = c("0" =  "grey", "1" = "black"),H3K4me3 = c("0" =  "grey", "1" = "black"),CTCF = c("0" =  "grey", "1" = "black"),hmec = c("0" =  "grey", "1" = "black"))  ,width = unit(1, "cm"))
top_anno_WGBS <- HeatmapAnnotation(df = data.frame(Subtype=c("G-CIMP low","G-CIMP high","G-CIMP high","Mesenchymal-like","Mesenchymal-like","LGm6-GBM","Non-tumor brain","Non-tumor brain")),
                                   col = list(Subtype = c("G-CIMP low" = "darkgreen", "G-CIMP high" = "firebrick3", "Mesenchymal-like" = "yellow","LGm6-GBM" = "cyan", "Non-tumor brain" = "gray")))
...
ha1 + 
  ha2 +
  Heatmap(WGBS.FC[,c(4,6,8,13,15,17,10,11)], name = "WGBS", col=jet.colors(75), cluster_rows = FALSE, cluster_columns = FALSE,show_row_names=FALSE,show_column_names = FALSE,top_annotation = top_anno_WGBS) 


aux <- melt(DM.633.Fig3E.hg38[,c(5:9)],id.vars="CpG.type")
levels(aux$variable) <- c("H3K27ac","H3K4me3","CTCF","5hmC")
aux$CpG.type <- factor(aux$CpG.type)
levels(aux$CpG.type) <- c("CpG island","Open-sea","CpG shore") 

ggplot(aux,aes(variable,fill=value)) +
  geom_bar() + 
  facet_grid(~ CpG.type) +
...
### Flavahan hg38 - CTCF
load("/media/data1/thais.projects/GCIMP-low/ActiveMotif_Data/DiffBind/DiffBind_homer.rda")
lost.ctcf.flavahan <- read.table("/media/data1/thais.projects/GCIMP-low/CTCF_Flavahan/FromHuy/lost_in_idh1mut_hg38.bed")
colnames(lost.ctcf.flavahan) <- c("chrom","start","end","id","score","strand")
ctcf.r <- dba.report(ctcf.a,th=1)
aux <- distanceToNearest(makeGRangesFromDataFrame(lost.ctcf.flavahan),ctcf.r)
lost.ctcf.gcimpLow <- ctcf.r[(elementMetadata(ctcf.r)[,4] > 0)]



### Checking DNA methylation levels at the CTCF binding site reported near PDGFRA
aux <- GRanges(seqnames = c("chr4","chr4"),ranges = IRanges(start = c(53908000,53929000), end=c(53910000,53931000)))
aux <- findOverlaps(makeGRangesFromDataFrame(LGG.GBM.WGBS),aux)
aux <- LGG.GBM.WGBS[queryHits(aux),]


aux <- GRanges(seqnames = c("chr4","chr4"),ranges = IRanges(start = c(53908000,53929000), end=c(53910000,53931000)))
aux <- findOverlaps(aux,makeGRangesFromDataFrame(hg38.450k))
probes <- DM.633.Fig3E.hg38[subjectHits(aux),]
probes.599.450k <- low.high.450k[as.character(probes$Composite.Element.REF),]
probes.599.WGBS <- LGG.GBM.WGBS[queryHits(aux),]
...
top_anno_WGBS <- HeatmapAnnotation(df = data.frame(Subtype=c("G-CIMP low","G-CIMP high","G-CIMP high","Mesenchymal-like","Mesenchymal-like","LGm6-GBM","Non-tumor brain","Non-tumor brain")),
  ...
  Heatmap(aux[,c(4,6,8,13,15,17,10,11)], name = "WGBS", col=jet.colors(75), cluster_rows = FALSE, cluster_columns = FALSE,show_row_names=FALSE,show_column_names = FALSE,top_annotation = top_anno_WGBS) 

### Checking DNA methylation levels at the all CTCF binding site reported to be lost in IDH mutant
aux <- findOverlaps(makeGRangesFromDataFrame(LGG.GBM.WGBS),makeGRangesFromDataFrame(lost.ctcf.flavahan))
cpg <- cbind(LGG.GBM.WGBS[queryHits(aux),],data.frame(CTCF.id=lost.ctcf.flavahan[subjectHits(aux),4]))

b <- cpg[cpg$Diff.low.high < -50 & !is.na(cpg$Diff.low.high),]
aux <- hclust(dist(b[,c(4,6,8,13,15,17,10,11)]))
hypo <- b[aux$order,]
b <- cpg[cpg$Diff.low.high > 50 & !is.na(cpg$Diff.low.high),]
aux <- hclust(dist(b[,c(4,6,8,13,15,17,10,11)]))
hyper <- b[aux$order,]
b <- cpg[cpg$Diff.low.high >= -50 & cpg$Diff.low.high <= 50 & !is.na(cpg$Diff.low.high),]
aux <- hclust(dist(b[,c(4,6,8,13,15,17,10,11)]))
nochange <- b[aux$order,]g
b <- rbind(hypo,hyper,nochange)

ha = rowAnnotation(df = data.frame(CpG.class=c(rep("Hypomethylated",nrow(hypo)),rep("Hypermethylated",nrow(hyper)),rep("No-change",nrow(nochange)))), col = list(CpG.class = c("Hypomethylated" = "darkslategray1", "Hypermethylated" = "darkslategray","No-change"="darkslategray4")), width = unit(1, "cm"))

ha + Heatmap(b[,c(4,6,8,13,15,17,10,11)], name = "WGBS", col=jet.colors(75), cluster_rows = FALSE, cluster_columns = FALSE,show_row_names=FALSE,show_column_names = FALSE,top_annotation = top_anno_WGBS)   

aux <- GRanges(seqnames = "chr12",ranges = IRanges(start = 57650000, end=57900000))
findOverlaps(aux,makeGRangesFromDataFrame(lost.ctcf.flavahan))

### Nearest genes to these DM CTCFs
ha + Heatmap(hypo[,c(4,6,8,13,15,17,10,11)], name = "WGBS", col=jet.colors(75), cluster_rows = FALSE, cluster_columns = FALSE,show_row_names=FALSE,show_column_names = FALSE,top_annotation = top_anno_WGBS)   

DM.CTCF <- lost.ctcf.flavahan[lost.ctcf.flavahan$id %in% as.character(hypo$CTCF.id),]
aux <- precede(makeGRangesFromDataFrame(DM.CTCF),makeGRangesFromDataFrame(genes))
aux <- genes[aux,]
#aux[aux$RNAseqALL.summary %in% "Upregulated",]
up <- aux[aux$RNAseq8.summary %in% "Downregulated",]

DM.CTCF <- lost.ctcf.flavahan[lost.ctcf.flavahan$id %in% as.character(hypo$CTCF.id),]
aux <- follow(makeGRangesFromDataFrame(DM.CTCF),makeGRangesFromDataFrame(genes))
aux <- genes[aux,]
#aux[aux$RNAseqALL.summary %in% "Upregulated",]
aux <- aux[aux$RNAseq8.summary %in% "Downregulated",]
up <- rbind(up,aux)
rownames(up) <- as.character(up$gene_name)

top_anno_exp <- HeatmapAnnotation(df = data.frame(Subtype=c("G-CIMP low","G-CIMP high","Non-tumor brain")),
                                   col = list(Subtype = c("G-CIMP low" = "darkgreen", "G-CIMP high" = "firebrick3", "Non-tumor brain" = "gray")))

Heatmap(up[,c(10,11,9)], name = "Gene Expression", col=greenred(75), cluster_rows = T, cluster_columns = FALSE,show_row_names=T,show_column_names = FALSE,top_annotation = top_anno_exp)   

ha + Heatmap(hyper[,c(4,6,8,13,15,17,10,11)], name = "WGBS", col=jet.colors(75), cluster_rows = FALSE, cluster_columns = FALSE,show_row_names=FALSE,show_column_names = FALSE,top_annotation = top_anno_WGBS)   

DM.CTCF <- lost.ctcf.flavahan[lost.ctcf.flavahan$id %in% as.character(hyper$CTCF.id),]
aux <- precede(makeGRangesFromDataFrame(DM.CTCF),makeGRangesFromDataFrame(genes))
aux <- genes[aux,]
#aux[aux$RNAseqALL.summary %in% "Upregulated",]
up <- aux[aux$RNAseq8.summary %in% "Upregulated",]

DM.CTCF <- lost.ctcf.flavahan[lost.ctcf.flavahan$id %in% as.character(hyper$CTCF.id),]
aux <- follow(makeGRangesFromDataFrame(DM.CTCF),makeGRangesFromDataFrame(genes))
aux <- genes[aux,]
#aux[aux$RNAseqALL.summary %in% "Upregulated",]
aux <- aux[aux$RNAseq8.summary %in% "Upregulated",]
up <- rbind(up,aux)
rownames(up) <- as.character(up$gene_name)
...
ctcf.r <- dba.report(ctcf.a,th=1)
aux <- distanceToNearest(makeGRangesFromDataFrame(lost.ctcf.flavahan),ctcf.r)
aux <- aux[mcols(aux)[,1] < 2000,] #360 out of 623
aux <- ctcf.r[subjectHits(aux),]
ov.ctcf <- aux[(mcols(aux)[,6] < 0.05),] #8 CTCFs

aux <- findOverlaps(ov.ctcf,makeGRangesFromDataFrame(hg38.450k))

top_anno_WGBS <- HeatmapAnnotation(df = data.frame(Subtype=c("G-CIMP low","G-CIMP high","G-CIMP high","Mesenchymal-like","Mesenchymal-like","LGm6-GBM","Non-tumor brain","Non-tumor brain")),
                                   col = list(Subtype = c("G-CIMP low" = "darkgreen", "G-CIMP high" = "firebrick3", "Mesenchymal-like" = "yellow","LGm6-GBM" = "cyan", "Non-tumor brain" = "gray")))

aux <- findOverlaps(makeGRangesFromDataFrame(LGG.GBM.WGBS),ov.ctcf)
aux <- as.data.frame(aux)
#cpg <- LGG.GBM.WGBS[aux[,"queryHits"],c(4,6,8)]
cpg <- LGG.GBM.WGBS[aux[aux$subjectHits == 5, "queryHits"],c(4,6,8,13,15,17,10,11)]
png("/media/data1/thais.projects/GCIMP-low/GBM.WGBS/633p_Fig_3E/heat5.png",width = 250,height = 250)
a <- Heatmap(cpg, name = "WGBS", col=jet.colors(75), cluster_rows = FALSE, cluster_columns = FALSE,show_row_names=FALSE,show_column_names = FALSE,top_annotation = top_anno_WGBS,show_heatmap_legend = F)   
draw(a,show_annotation_legend = FALSE)
dev.off()

aux <- ov.ctcf[c(2,4),] #2 CTCF - PADI4 is downregulated
#distanceToNearest(aux,makeGRangesFromDataFrame(genes[genes$gene_name %in% "PADI4",]))

## Genes within 500k nearby these 8 CTCF binding sites
ctcfs <- ov.ctcf
start(ctcfs) <- ((start(ov.ctcf)+end(ov.ctcf))/2) - 500000
end(ctcfs) <- ((start(ov.ctcf)+end(ov.ctcf))/2) + 500000
aux <- findOverlaps(ctcfs,makeGRangesFromDataFrame(genes))
aux <- genes[subjectHits(aux),]
aux <- aux[aux$RNAseq8.summary %in% c("Downregulated","Upregulated") | aux$RNAseqALL.summary %in% c("Downregulated","Upregulated"),]
table(aux$RNAseq8.summary,aux$RNAseqALL.summary)
rownames(aux) <- as.character(aux$gene_name)

top_anno_exp <- HeatmapAnnotation(df = data.frame(Subtype=c("G-CIMP low","G-CIMP high","Non-tumor brain")),
                                  col = list(Subtype = c("G-CIMP low" = "darkgreen", "G-CIMP high" = "firebrick3", "Non-tumor brain" = "gray")))

ha = rowAnnotation(df = data.frame(Gene.classification=aux$RNAseq8.summary), col = list(Gene.classification = c("Downregulated" = "darkslategray1", "Upregulated" = "darkslategray","No.difference"="cadetblue")), width = unit(1, "cm"))
ha1 = rowAnnotation(df = data.frame(Gene.classification=aux$RNAseqALL.summary), col = list(Gene.classification = c("Downregulated" = "darkslategray1", "Upregulated" = "darkslategray","No.difference"="cadetblue","No.info"="bisque1")), width = unit(1, "cm"))

ha + 
Heatmap(aux[,c(10,11,9)], name = "Gene Expression", col=greenred(75), cluster_rows = T, cluster_columns = FALSE,show_row_names=F,show_column_names = FALSE,top_annotation = top_anno_exp,column_title = "G-CIMP low and G-CIMP which we profiled ChIP-seq (n=8)")  +  
  ha1 + 
Heatmap(aux[,c(22,23,21)], name = "Gene Expression", col=greenred(75), cluster_columns = FALSE,show_row_names=T,show_column_names = FALSE,top_annotation = top_anno_exp,column_title = "All G-CIMP low and G-CIMP high samples from TCGA (n=252)") 

## Find the actual CTCF binding site among this 8 peaks
aux <- as.data.frame(ov.ctcf)
aux$id <- 1:nrow(aux)
write.table(aux[,c(1,2,3,12)],quote=F,row.names = F,col.names = F,sep="\t",file="/media/data1/thais.projects/GCIMP-low/ActiveMotif_Data/homer.motifs/CTCF_8motifs/8_motifs.txt")



##### hox genes
symbol <- grep("^HOX[ACD]+",rownames(rna.seq.LGG.GBM.normalBrain))

normal.mrna <- rna.seq.LGG.GBM.normalBrain[symbol,1:5]
normal.mrna <- apply(normal.mrna,1,mean,na.rm=TRUE)
normal.mrna <- as.data.frame(normal.mrna)
colnames(normal.mrna) <- "value"
normal.mrna$type <- "Non-tumor Brain"
aux <- as.character(subset(pd, cartoon %in% c("GCIMP-low"))$case.id)
low.mrna <- rna.seq.LGG.GBM.normalBrain[symbol,colnames(rna.seq.LGG.GBM.normalBrain) %in% aux]
low.mrna <- apply(low.mrna,1,mean,na.rm=TRUE)
low.mrna <- as.data.frame(low.mrna)
colnames(low.mrna) <- "value"
low.mrna$type <- "GCIMP-low"
aux <- as.character(subset(pd, cartoon %in% c("GCIMP-high"))$case.id)
high.mrna <- rna.seq.LGG.GBM.normalBrain[symbol,colnames(rna.seq.LGG.GBM.normalBrain) %in% aux]
high.mrna <- apply(high.mrna,1,mean,na.rm=TRUE)
high.mrna <- as.data.frame(high.mrna)
colnames(high.mrna) <- "value"
high.mrna$type <- "GCIMP-high"
aux <- as.character(subset(pd, cartoon %in% c("IDHmut-codel"))$case.id)
codel.mrna <- rna.seq.LGG.GBM.normalBrain[symbol,colnames(rna.seq.LGG.GBM.normalBrain) %in% aux]
codel.mrna <- apply(codel.mrna,1,mean,na.rm=TRUE)
codel.mrna <- as.data.frame(codel.mrna)
colnames(codel.mrna) <- "value"
codel.mrna$type <- "IDHmut-codel"
aux <- as.character(subset(pd, cartoon %in% c("Classic-like"))$case.id)
classic.mrna <- rna.seq.LGG.GBM.normalBrain[symbol,colnames(rna.seq.LGG.GBM.normalBrain) %in% aux]
classic.mrna <- apply(classic.mrna,1,mean,na.rm=TRUE)
classic.mrna <- as.data.frame(classic.mrna)
colnames(classic.mrna) <- "value"
classic.mrna$type <- "Classic-like"
aux <- as.character(subset(pd, cartoon %in% c("Mesenchymal-like"))$case.id)
mes.mrna <- rna.seq.LGG.GBM.normalBrain[symbol,colnames(rna.seq.LGG.GBM.normalBrain) %in% aux]
mes.mrna <- apply(mes.mrna,1,mean,na.rm=TRUE)
mes.mrna <- as.data.frame(mes.mrna)
colnames(mes.mrna) <- "value"
mes.mrna$type <- "Mesenchymal-like"
aux <- as.character(subset(pd, cartoon %in% c("LGm6-GBM"))$case.id)
lgm6.mrna <- rna.seq.LGG.GBM.normalBrain[symbol,colnames(rna.seq.LGG.GBM.normalBrain) %in% aux]
lgm6.mrna <- apply(lgm6.mrna,1,mean,na.rm=TRUE)
lgm6.mrna <- as.data.frame(lgm6.mrna)
colnames(lgm6.mrna) <- "value"
lgm6.mrna$type <- "LGm6-GBM"
aux <- as.character(subset(pd, cartoon %in% c("PA-like"))$case.id)
pa.mrna <- rna.seq.LGG.GBM.normalBrain[symbol,colnames(rna.seq.LGG.GBM.normalBrain) %in% aux]
pa.mrna <- apply(pa.mrna,1,mean,na.rm=TRUE)
pa.mrna <- as.data.frame(pa.mrna)
colnames(pa.mrna) <- "value"
pa.mrna$type <- "PA-like"

aux <- rbind(normal.mrna,low.mrna,high.mrna)
aux$hox.family <- "HOXD"
hox <- rbind(hox,aux)


hox <- rbind(normal.mrna,low.mrna,high.mrna,codel.mrna,classic.mrna,mes.mrna,lgm6.mrna,pa.mrna)
hox$hox.family <- "HOX genes"

hox$type <- as.factor(hox$type)
hox$type <- factor(hox$type, levels = c("Non-tumor Brain","GCIMP-low","GCIMP-high","IDHmut-codel","Classic-like","Mesenchymal-like","LGm6-GBM","PA-like"))
ggplot(hox, aes(type, value,fill = type)) +
  geom_boxplot(outlier.shape=NA)  +
  geom_jitter() +
  ylab("RNAseq values (log2 transformed") +
  xlab("Glioma subtypes") +
  facet_wrap( ~ hox.family,scales="free") +
  theme(axis.text.x = element_blank(),strip.text = element_text(size=15),axis.text.y = element_text(size=10),legend.text=element_text(size=10),legend.title=element_text(size=10)) +
  scale_fill_manual(values = c("gray","darkgreen","firebrick","#9f20f0","#ffa500","#FFFF00","#006680","#00CCFF"),name="Glioma subtypes")


aux <- rna.seq.LGG.GBM.normalBrain[,642:649]
aux$type <- NA
aux[aux$p.value.adj < 0.05 & !is.na(aux$p.value.adj) & aux$DiffMean < 0,"type"] <- "Downregulated"
aux[aux$p.value.adj < 0.05 & !is.na(aux$p.value.adj) & aux$DiffMean > 0,"type"] <- "Upregulated"
write.table(aux[aux$type %in% "Downregulated","entrezID"],quote=F,sep="\t",col.names=F,row.names=F,file="/media/data1/thais.projects/GCIMP-low/ActiveMotif_Data/homer.motifs/DiffBind_H3K27ac_gained/TAAT_motif/Genes/Down.txt")
write.table(aux[aux$type %in% "Upregulated","entrezID"],quote=F,sep="\t",col.names=F,row.names=F,file="/media/data1/thais.projects/GCIMP-low/ActiveMotif_Data/homer.motifs/DiffBind_H3K27ac_gained/TAAT_motif/Genes/Up.txt")



sig <- read.delim("/media/data1/thais.projects/GCIMP-low/ActiveMotif_Data/homer.motifs/DiffBind_H3K27ac_gained/TAAT_motif/Genes/teste.txt")

#### BED files
#LGG.GBM.WGBS$id <- paste0("chr:",LGG.GBM.WGBS$id)
aux <- LGG.GBM.WGBS[,c("chrom","start","end","id","percentMeth.0128")]
aux$strand <- "*"
aux[,5] <- aux[,5]/100
write.table(aux,quote=F,col.names=F,row.names=F,sep="\t",file="/media/data1/thais.projects/GCIMP-low/GBM.WGBS/Visualization/TCGA.06.0128.01A.bed")
a <- import(con="TCGA.06.0128.01A.bed",format="bed")
...
aux[,5] <- aux[,5]/100
write.table(aux,quote=F,col.names=F,row.names=F,sep="\t",file="/media/data1/thais.projects/GCIMP-low/GBM.WGBS/Visualization/TCGA.06.0128.01A.bed")
aux <- LGG.GBM.WGBS[,c("chrom","start","end","id","percentMeth.1460")]
aux$strand <- "+"
aux[,5] <- aux[,5]/100
write.table(aux,quote=F,col.names=F,row.names=F,sep="\t",file="/media/data1/thais.projects/GCIMP-low/GBM.WGBS/Visualization/TCGA.16.1460.01A.bed")
