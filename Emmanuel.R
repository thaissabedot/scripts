...
gr <- GRanges(seqnames = "chr11",ranges = IRanges(start = 68206893, end=68216884))
...
pdf("/home/thais/GAL_heatmap_den.pdf",width=15, height=10)
h1+h2
dev.off()
...
#hg19
a <- merge(all.450k[,1:4],gset.expr,by.x="Composite.Element.REF",by.y=0)
rownames(a) <- as.character(a$Composite.Element.REF)
gr <- GRanges(seqnames = "chr17",ranges = IRanges(start = 74069000, end=74074200))


b <- subsetByOverlaps(makeGRangesFromDataFrame(a,
                                     ...
                                     starts.in.df.are.0based=FALSE),gr)

b <- as.data.frame(b)
b <- b[with(b,order(start)),]
c <- melt(b[,-c(1:5,7)])
c$group <- c(rep("MS",504),rep("Control",342))

ggplot(c,aes(group,value,fill=group)) +
  geom_boxplot() + ylim(0,1) +
  scale_fill_manual(values = c("gray30","red")) +
  facet_wrap(~ Composite.Element.REF)

z <- seq(min(b[,-c(1:7)],na.rm=TRUE), max(b[,-c(1:7)],na.rm=TRUE), length = 200) #min(lgg.gbm.27.450.order) and #max(lgg.gbm.27.450.order)
n <- max(b[,-c(1:7)],na.rm=TRUE)

image(matrix(z, ncol = 1), col = jet.colors(75),
      xaxt = "n", yaxt = "n", main = "Overlap count Key β")
box()
par(usr = c(0, n, 0, n))
